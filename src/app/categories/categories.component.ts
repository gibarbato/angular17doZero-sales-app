...
export class CategoriesComponent implements AfterViewInit {
  ...
  async loadCategories(): Promise<void> {
    ...
    const categories = await lastValueFrom(this.categoriesService.getAll());
  ...
  async onSave(category: Category) {
    const saved = lastValueFrom(this.categoriesService.save(category));
  ...
  async onDeleteCategoryClick(category: Category) {
    if (confirm(`Deletar "${category.name}" com id ${category.id} ?`)) {
      this.showLoading = true;
      await lastValueFrom(this.categoriesService.delete(category.id));
      this.showLoading = false;
      this.loadCategories();
    }
  }
